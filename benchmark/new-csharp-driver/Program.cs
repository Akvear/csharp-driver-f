...
var uri        = Environment.GetEnvironmentVariable("SCYLLA_URI") ?? "127.0.0.1";
var iterations = int.TryParse(Environment.GetEnvironmentVariable("ITERATIONS"), out var i) ? i : 10_000;

Console.WriteLine($"Driver    : ScyllaDB C# Driver (Rust-backed)");
Console.WriteLine($"Benchmark : {benchmark}");
Console.WriteLine($"Connecting to {uri}...");

var cluster = Cluster.Builder().AddContactPoint(uri).Build();
